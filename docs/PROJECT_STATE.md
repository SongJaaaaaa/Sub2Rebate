...
2026-06-16 已完成 Sub2API 用户删号与邀请树风险专项分析，并补充测试验证计划；新增 `6/16 问题清单/Sub2API删号与邀请树问题清单-2026-06-16.md`，明确本地用户节点不能跟随上游硬删，后续需补上 tombstone 生命周期、上游删号对账、返利可用上级过滤，以及删号场景的自动化 / 联调 / 回归验收用例。

2026-06-17 已确认返利层级优化方向：邀请树是历史结构，中间节点失效时下级不自动上浮；失效节点在管理端置灰但仍可登录；账号登录状态和返利资格需要拆分；失效节点不获得新的里程碑或多级返利；多级返利中失效节点金额由配置决定，支持“归平台”和“排除失效节点后按衰减系数重算”。本次同时纳入轻量防躺平：监控充值、账户余额减少和发展下级，连续 7 天无活跃则置灰并暂停返利资格，有充值后恢复。原 `D:\test项目\分销\6` 目录已改为 `D:\test项目\分销\日报`，后续日报与问题清单按新目录维护。
...
- 邀请树失效节点：中间节点被删除、失联或后续被防躺平判定为返利失效时，下级不上浮，`referral_paths` 保留历史结构，管理端置灰。
- 账号状态与返利资格拆分：失效节点默认仍可登录，但不获得新的返利；封禁才影响登录。
- 失效节点返利处理：新增配置 `rebate.inactive_node_mode`，默认建议 `platform` 归平台，也可切换 `exclude_recalculate` 排除失效节点后重算。
- 轻量防躺平：默认连续 7 天没有充值、账户余额没有减少、没有发展新下级，则 `rebate_status = disabled` 并置灰；用户有充值后恢复 `eligible`。当前 `Sub2ApiUserRepository` 已能读取 Sub2API `users.balance` / `users.total_recharged`，余额下降可作为活跃使用信号，总充值增加可作为恢复信号；余额变化只做活跃判断，不作为返利事件来源。
- 返利池比例：默认 15%，可配置。
...
- 里程碑奖励最终是否继续奖励直接上级，还是调整为纯个人权益；当前代码仍是奖励直接上级，2026-06-17 讨论中明确里程碑不参与多级层级返利。
...
1. 优先补上 Sub2API 用户删号/失联对账、本地 tombstone 生命周期和返利可用上级过滤，避免邀请树与账务因硬删失真。
2. 落地 `rebate_status`、`rebate.inactive_node_mode`、轻量防躺平配置、失效节点置灰、两种返利计算模式和专项测试。
3. 继续 B11 Sub2API 扫描集成：`payment_orders`、`redeem_codes`、`payment_audit_logs` 转换为幂等 `rebate_events`。
4. 继续 B10 复杂后台页面：邀请关系可视化、用户余额调整弹窗、充值事件补录入口。
5. 继续前端用户端页面和 API 联调。
