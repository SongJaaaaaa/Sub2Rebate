...
2026-06-15 修复 Sub2API 删除下级账号后 Sub2Rebate 仍显示旧邀请关系：推广中心、邀请树和邀请记录查询会重新读取 Sub2API 当前用户，并按 `inviter_id` 发现新增直接下级；上游明确不存在的本地下级账号会从本地 `referral_paths` 移出其分支，上游查询异常时保留本地路径避免误清。验证通过：`InviteTest|DeepDataSyncTest|DeepApiEndpointTest` 共 38 个测试、176 个断言。真实多层级邀请深测已完成：使用 Desert 的 Sub2API 邀请码 `8UDG84TQD7BD` 真实注册 A 主干 31 个账号（5 层全二叉，层级 1-5 为 1/2/4/8/16），补充 B 主干 15 个账号（4 层全二叉，层级 1-4 为 1/2/4/8）。Sub2API 注册开关已恢复 `registration_enabled = false`；本地推广中心页面显示 Desert 直邀 3、团队 47，并能看到 A/B 等新建节点。当前本机 `127.0.0.1:15432` 只读库隧道连接拒绝，真实删除账号的线上连通验证需恢复隧道后补跑；删除分支逻辑已由 Feature Test 覆盖。

B3 配置中心后端读取基础已落地：`config_items`、默认配置、`ConfigService` 和 `GET /api/v1/config/items` 已可用；Filament 配置修改和审计后续在 B10 后台阶段实现。

B4 邀请关系后端基础已落地：邀请码、绑定、上级链路、邀请树和邀请记录接口已可用；查询推广中心和树/记录时会刷新 Sub2API 当前团队，清理上游明确已删除的下级分支，查询异常时保留本地数据；邀请记录里的充值/返利金额等待查询逻辑接入真实流水后补齐。
...
2. 继续 B11 Sub2API 扫描集成：`payment_orders`、`redeem_codes`、`payment_audit_logs` 转换为幂等 `rebate_events`。
3. 继续 B10 复杂后台页面：邀请关系可视化、用户余额调整弹窗、充值事件补录入口。
4. 继续前端用户端页面和 API 联调。
