    ...
    <body>
        <div class="container">
            ...
            <div class="buttons">
                ...
                <button class="btn btn-secondary" onclick="checkOrderStatus(true)">刷新支付状态</button>
            </div>
        </div>
        ...
        <script>
            ...
            async function checkOrderStatus(runMonitor = false) {
                if (paid || checking) {
                    return;
                }
                ...
                try {
                    if (runMonitor) {
                        statusElement.textContent = '正在确认支付结果...';
                        await fetch('/api.php?action=monitor&internal_trigger=force_start', { cache: 'no-store' });
                    }

                    const response = await fetch('/api.php?action=order&out_trade_no=' + encodeURIComponent(outTradeNo) + '&pid=' + encodeURIComponent(pid), { cache: 'no-store' });
            ...
            // Periodically trigger monitoring and then check order status.
