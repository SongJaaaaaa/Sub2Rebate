...
class CodePay
{
    ...
    public function createPayment(array $params): array
    {
        ...
        try {
            ...
            if ($existingOrder) {
                $this->logger->info('Pending order already exists, reusing payment.', [
                    'trade_no' => $existingOrder['id'],
                    'out_trade_no' => $params['out_trade_no']
                ]);

                return $this->buildPaymentResponse($params, $existingOrder, $businessQrMode);
            }
            ...
            if ($businessQrMode) {
                ...
                $qrVersion = filemtime($qrCodePath) ?: time();
                $qrCodeUrl = $baseUrl . '/qrcode.php?type=business&token=' . $token . '&v=' . $qrVersion;
    ...
    private function buildPaymentResponse(array $params, array $order, bool $businessQrMode): array
    {
        ...
        $response = [
            ...
            'money' => number_format($originalAmount, 2, '.', ''),
            'payment_amount' => $paymentAmount,
            'payment_url' => $paymentUrl
        ];
        ...
        if ($businessQrMode) {
            ...
            $token = md5('qrcode_access_' . date('Y-m-d'));
            $qrVersion = filemtime($qrCodePath) ?: time();
            $response['qr_code_url'] = $this->getBaseUrl() . '/qrcode.php?type=business&token=' . $token . '&v=' . $qrVersion;
            $response['business_qr_mode'] = true;
            $response['payment_instruction'] = "请使用支付宝扫描二维码，支付金额：{$paymentAmount} 元";
            $response['payment_tips'] = [
                "请务必支付准确金额：{$paymentAmount} 元",
            ...
            $response['qr_code'] = $qrCodeGenerator->generate($paymentUrl);
        }
