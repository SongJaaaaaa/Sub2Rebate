...
class BillQuery
{
    ...
    private function initializeApiInstance(): void
    {
        ...
        $clientOptions = [
            ...
            'connect_timeout' => $config['http_connect_timeout'] ?? 5,
        ];

        $caCert = $config['ca_cert'] ?? (__DIR__ . '/../../data/cacert.pem');
        if (is_file($caCert)) {
            $clientOptions['verify'] = $caCert;
        }

        $this->apiInstance = new AlipayDataBillAccountlogApi(new Client($clientOptions));
