...
class PaymentMonitor
{
    ...
    public function runMonitoringCycle(): array
    {
        ...
        try {
            ...
            if (!$result['success']) {
                ...
                return [
                    'success' => false,
                    'status' => 'error',
                    'message' => $result['message'] ?? 'Alipay API returned an error.',
            ...
            if (empty($bills)) {
                ...
                return [
                    'success' => true,
                    'status' => 'completed',
                    'message' => "No recent payment bills found in the last {$minutes} minutes.",
                    'range' => "{$startTime} ~ {$endTime}",
                    'bills_count' => 0,
                    'matched_orders' => 0
                ];
            ...
            if ($businessQrMode) {
                $matchedOrders = $this->processBillsForBusinessQrMode($bills);
            } else {
                $matchedOrders = $this->processBillsForTraditionalMode($bills);
            }

            // 匹配完成后再清理过期订单，避免刚支付但稍晚到账的订单被提前删除
        ...
        return [
            'success' => true,
            'status' => 'completed',
            'message' => 'Payment monitoring cycle completed.',
            'range' => "{$startTime} ~ {$endTime}",
            'bills_count' => count($bills),
            'matched_orders' => $matchedOrders ?? 0
        ];
    }
    ...
    private function processBillsForBusinessQrMode(array $bills): int
    {
        ...
        foreach ($bills as $bill) {
            ...
            foreach ($orders as $pendingOrder) {
                $orderTime = strtotime($pendingOrder['add_time']);
                ...
                if ($billTime < $orderTime || ($billTime - $orderTime) > $tolerance) {
                    $this->logger->warning("Order found for amount {$billAmount}, but it is outside the time tolerance.", [
                        ...
                        'order_time' => $pendingOrder['add_time'],
            ...
            if (!$order) {
                $this->logger->info("No pending order within time tolerance for amount {$billAmount}. Skipping.");
                continue;
            }
            ...
            // 使用事务确保原子性
            $this->db->action(function($db) use ($order, &$matched) {
    ...
    private function processBillsForTraditionalMode(array $bills): int
    {
    ...
    private function notifyUser($order)
    {
        ...
        $host = parse_url($order['notify_url'], PHP_URL_HOST);
        if (in_array($host, ['127.0.0.1', 'localhost'], true)) {
            $this->logger->info("Local notify_url detected for order {$order['id']}. Skipping HTTP callback in local test.");
            return;
        }
