 ...
 * 改进的监控服务启动器 - 生产环境优化版本
 */
function startMonitoringServiceIfNeeded(): array {
    static $lastCheck = 0;
    ...
    if (!$forceStart && $currentTime - $lastCheck < $checkInterval) {
        ...
        return [
            'success' => true,
            'status' => 'skipped',
            'message' => 'Monitoring check skipped due to interval limit'
        ];
    }
    ...
    if (!$forceStart) {
        ...
        if (!$lockManager->tryLock()) {
            ...
            return [
                'success' => false,
                'status' => 'running',
                'message' => 'Monitoring already running, lock not acquired'
            ];
        }
    }
    ...
    try {
        ...
        $result = $paymentMonitor->runMonitoringCycle();
        if ($result['success']) {
            $logger->info('Container monitoring cycle completed successfully');
            updateMonitorStatus('completed', $result['message'] ?? 'API triggered monitoring cycle completed successfully');
        } else {
            $logger->error('Container monitoring cycle failed', ['message' => $result['message'] ?? 'unknown']);
            updateMonitorStatus('error', $result['message'] ?? 'Monitoring cycle failed');
        }
...
try {
    ...
    switch ($action) {
        ...
        case 'monitor':
            $result = startMonitoringServiceIfNeeded();
            $result = [
